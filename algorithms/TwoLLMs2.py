...
alg_name =  'TwoLLMs2'     # Difference with TwoLLMs is the scaling of update of c    (here it is scaled by *normalizer_w[:-1])

alpha = 1e-3
alpha1 = "alpha/2" # stepsize of the first layer
RMSPw = .99
RMSPc = .99
...
if __name__ == "__main__":
    ...
    if alpha1=='alpha':
         alpha1 = alpha
    elif isinstance(alpha1,str) and alpha1.split('/')[0]=='alpha':
        alpha1 =alpha/float(alpha1.split('/')[1])
    ...
    for iteration in range(num_steps):
        ...
        try:
            ...
            trace_abs_x = RMSPc*trace_abs_x + (1-RMSPc)* np.abs(xb[:-1])
            
            obn_coeff_for_w = min(1, 1/(alpha*xb*xb / normalizer_w).sum())
            
            #c = c - alpha1 * delta * wb[:-1] / normalizer_c # This is the gradient update of the first layer weights
            c = c - alpha1 * delta * wb[:-1] * trace_abs_x / normalizer_c 
            wb = wb + obn_coeff_for_w * alpha *  delta * xb / normalizer_w
