...
alg_name =  'IDBD_comb' 
base_alg = 'LMS_GMDN'
base_alg_params = {'theta_MDN':'theta_meta', 'eta0':.001, 'transform':'pre:signW,post:v@0.001'} #{'theta_MDN':.001, 'eta0':.001} # {} {'theta_MDN':.001, 'eta':.001}

alpha0 = 1e-4
meta_stepsize = 1e-3
meta_eta = 1e-3
gamma = 1.0
delta_norm =  'none' # 'none'  or 'eta' or '.1eta'
    ...
    for bias in ['True']:
        for gamma in [1.0]:
            for base_alg, base_alg_params_list, alpha0_list in [
                                    ...
                                    #('LMS_GMDN', [{'theta_MDN':'theta_meta', 'eta0':.001, 'transform':'pre:none,post:both@0.01@0.01'}], [1e-5]),
                                    #('LMS_GMDN', [{'theta_MDN':'theta_meta/10', 'eta0':.001, 'transform':'pre:none,post:both@0.01@0.01'}], [1e-5]),
...
if __name__ == "__main__":
    if meta_eta == 'theta_meta':
        meta_eta = meta_stepsize + 0.0
    elif meta_eta == 'theta_meta/10':
        meta_eta = meta_stepsize / 10.0
    else:
        meta_eta = meta_eta + 0.0
    ...
    for iteration in range(num_steps):
        ...
        try:
            ...
            # meta update
            if delta_norm=='eta':
                N_delta = N_delta + (1-meta_eta) * (delta**2-N_delta) 
                delta_tilde = delta / np.clip(np.sqrt(N_delta), a_min=1e-8, a_max=None)
            if delta_norm=='.1eta':
                N_delta = N_delta + (1-0.1*meta_eta) * (delta**2-N_delta) 
                delta_tilde = delta / np.clip(np.sqrt(N_delta), a_min=1e-8, a_max=None)
            elif delta_norm=='none':
                delta_tilde = delta + 0.0
            
            coeff_meta_nu = meta_eta/(1-(1-meta_eta)**t)
            meta_grad = -delta_tilde*xb*h
            meta_nu += coeff_meta_nu*(meta_grad**2-meta_nu)
            beta = beta - meta_stepsize* meta_grad / (np.sqrt(meta_nu)+1e-8)
            alpha = np.exp(beta)
            h = h * clip(1-alpha* xb**2) + alpha * delta*xb
