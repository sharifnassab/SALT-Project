...
meta_stepsize = 1e-3
gamma = 1.0
beta_inc_clip = "none" # "none"  or float
meta_post_norm = 0.01 # "none"  or float
delta_norm = "False" # "True", "False"
beta_pre_norm = "none" # "steady", "none"
h_pre_norm = "steady" # "steady", "none", "alpha" "__1-alpha*x2__alpha"
h_decay = "1-alpha*x2"   #"steady", "none", "1-alpha*x2", 
                    ...
                    for meta_stepsize in [1e-4,1e-3,1e-2]:
                        for beta_inc_clip in [ 
                            ...
                            for meta_post_norm in [
                                ...
                                for delta_norm in ["False"]: #"True", "False"
                                    for h_decay in [
                                                #0.0,
                                                #0.9,
                                                #0.99,
                                        ...
                                        for beta_pre_norm in ["none"]: #"steady", "none"
                                            for h_pre_norm in [ #"steady", "none", "alpha", "__1-alpha*x2__alpha"
                                                                "alpha",
                                                                "__1-alpha*x2__alpha",  # default
                                                                ]: 
...
def softmax_k1(b,x):
    exp_z = np.exp(x-np.median(b))  # median is substracted for numerical statibility and avoiding exp blow-up
    denom = (exp_z*(x**2)).sum()
    return exp_z[:-1]/denom

if __name__ == "__main__":
    env, num_steps, d = data_loader(dataset)
    try:
        beta_inc_clip = float(beta_inc_clip)
    except:
        if beta_inc_clip == 'none':
            beta_inc_clip = 1e8
        else:
            raise(ValueError)
    ...
    alpha = softmax_k1(beta, np.ones_like(beta))
    h = 0.0
    ...
    for iteration in range(num_steps):
        ...
        if 1:
            ...
            if delta_norm in ["True", True]:
                delta_tilde = delta / clip_eps(np.sqrt(N))
            elif delta_norm in ["False", False]:
                delta_tilde = delta + 0.0
            else:
                raise(ValueError)

            if beta_pre_norm == 'steady':
                beta_prenorm_coeff = np.sqrt(2*alpha[:-1]-alpha[:-1]**2)
            elif beta_pre_norm == "none":
                beta_prenorm_coeff = 1.0
            else:
                raise(ValueError)


            # post normalization of meta step:
            meta_beta_increment = delta_tilde*beta_prenorm_coeff  *  xb*h 
            if meta_post_norm is None:
                current_meta_beta_increment = meta_beta_increment
            else:
                ...
                #current_meta_beta_increment = meta_beta_increment / np.clip(np.sqrt(trace_meta_post_norm/(1-(1-meta_post_norm)**t)), a_min=1e-8, a_max=None)
            
            beta[:-1] = beta[:-1] + np.clip(meta_stepsize * current_meta_beta_increment, a_min=None, a_max=beta_inc_clip) 
            alpha = softmax_k1( np.clip(beta, a_min=None, a_max=25) , np.concatenate((xb,np.array([1.0]))) )

            if h_pre_norm == "steady":
                h_prenorm_coeff = np.sqrt(2*alpha-alpha**2)
            elif h_pre_norm == "none":
                h_prenorm_coeff = 1.0
            elif h_pre_norm == "alpha":
                h_prenorm_coeff = alpha
            elif h_pre_norm == "__1-alpha*x2__alpha":
                h_prenorm_coeff = clip0(1-alpha*xb*xb)*alpha
            else:
                raise(ValueError)
            ...
            h_decay_coeff = (
                            ...
                            clip0(1-alpha*xb*xb) if h_decay=='1-alpha*x2' else
                            0/0)
            h = h * h_decay_coeff + h_prenorm_coeff * delta_tilde*xb
            
            # base update
            wb = wb + alpha * delta_tilde*xb
