...
if __name__ == "__main__":
    ...
    for iteration in range(num_steps):
        ...
        if 1:
            ...
            obn_coeff = alpha/((xb*xb / normalizer_w).sum() + (a1_to_a2_ratio * wb[:-1] * wb[:-1] *trace_abs_x / normalizer_c).sum())
            
            delta_c =  - obn_coeff * a1_to_a2_ratio * delta * wb[:-1] * trace_abs_x / normalizer_c # This is the gradient update of the first layer weights
            #c = c - alpha1 * delta * wb[:-1] *normalizer_w[:-1]/ normalizer_c 
            delta_w =  obn_coeff *  delta * xb / normalizer_w

            if beta_m:
                m_c = beta_m * m_c + (1-beta_m) * delta_c
                m_w = beta_m * m_w + (1-beta_m) * delta_w
            else:
                m_c = delta_c
                m_w = delta_w
