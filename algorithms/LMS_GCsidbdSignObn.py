...
alg_name =  'LMS_GCsidbdSignObn'      # goal directed centering with sidbd
...
class GCsidbdSignObn():
    ...
    def step(self, x, z, w, b):
        ...
        obn_coeff_for_mu = 1/(np.abs(w).sum()+1e-8)
        self.mu = self.mu - alpha_mu * delta * np.sign(w) * obn_coeff_for_mu

        h_projection =  (w*self.h_mu).sum() * alpha_mu* np.sign(w) * obn_coeff_for_mu
        h_overshoor_bound = min([1, np.sqrt(np.sum(self.h_mu**2) / ((h_projection**2).sum()+1e-16))])
        self.h_mu = self.h_mu - h_overshoor_bound*h_projection + delta* np.sign(w) * obn_coeff_for_mu
...
if __name__ == "__main__":
    ...
    wb = np.zeros(d+1)
    input_normalizer = GCsidbdSignObn(meta_stepsize_mu=meta_stepsize_mu, meta_post_norm_mu=meta_post_norm_mu, alpha_mu_0=alpha_mu_0)
    MSE_list = []
    t = 0
