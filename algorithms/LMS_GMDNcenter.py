...
alg_name =  'LMS-GMDNcenter' 
alpha = 1e-4
eta0 = .001
theta_MDN = 1e-4
transform = 'pre:none,post:0.01'  # 'None' 'signW'  'pre:none,post:0.01'
...
class GMDNcenter():
    def __init__(self, theta, eta0, transform='None', activation='sigmoid'):
        ...
        if transform.split(',')[1].split(':')[1].lower() in ['none']:
            ...
            self.post_norm_type = transform.split(',')[1].split(':')[1]
...
def main_loop(dataset=dataset, alpha=alpha, eta0=eta0, theta_MDN=theta_MDN, bias=bias):
    ...
    input_normalizer = GMDNcenter(theta=theta_MDN, eta0=eta0, transform=transform)
    MSE_list = []
