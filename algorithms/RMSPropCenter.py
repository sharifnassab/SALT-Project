...
def param_sweeps():
    ...
    for bias in ['True']: # ['False', 'True']
        for grad_normalizer_param in [.99]: # [.999, .99]
            for input_centering_param in [.99]:
...
class RMSProp():
    def __init__(self, alpha, grad_normalizer_param=0.999, w0=0.0):
    ...
    def step(self, grad):
        ...
        self.w = self.w - self.alpha * grad/v
        return self.w
...
if __name__ == "__main__":
    ...
    optimizer = RMSProp(alpha, grad_normalizer_param, w0=w)
    centering = input_cenetring(input_centering_param)
    MSE_list = []
