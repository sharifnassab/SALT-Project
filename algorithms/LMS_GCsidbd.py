...
alg_name =  'LMS_GCsidbd'      # goal directed centering with sidbd

alpha = 1e-5
alpha_mu_0 = "alpha/2" # stepsize of the first layer
...
class GCsidbd():
    ...
    def step(self, x, z, w, b):
        ...
        alpha_mu = act_exp(self.beta_mu)

        self.mu = self.mu - alpha_mu * delta * w

        #norm_x = (w*w).sum() # for  Overshoot bounding
        h_projection =  (w*self.h_mu).sum() * alpha_mu*w
        h_overshoor_bound = min([1, np.sqrt(np.sum(self.h_mu**2) / ((h_projection**2).sum()+1e-16))])
        self.h_mu = self.h_mu - h_overshoor_bound*h_projection + delta*w
...
if __name__ == "__main__":
    ...
    wb = np.zeros(d+1)
    input_normalizer = GCsidbd(meta_stepsize_mu=meta_stepsize_mu, meta_post_norm_mu=meta_post_norm_mu, alpha_mu_0=alpha_mu_0)
    MSE_list = []
    t = 0
