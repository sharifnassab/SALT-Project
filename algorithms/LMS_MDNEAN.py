...
class MDNEAN():
    def __init__(self, theta_mu, eta_v, eta0_mu=None, h_decay="1-alpha", activation='sigmoid'):
    ...
    def step(self, x):
        ...
        self.beta_mu = self.beta_mu + self.theta_mu *x_tilde *self.h_mu 

        h_decay_coeff = (
                            h_decay if isinstance(h_decay, float) else
                            (1-eta_mu) if h_decay=='1-alpha' else
                            0/0)
        self.h_mu = h_decay_coeff * self.h_mu  + x_tilde
        return x_tilde, self.mu, sigma



def run_once(dataset=dataset, alpha=alpha, eta_v=eta_v, theta_MDN=theta_MDN, h_decay=h_decay, bias=bias):
    env, num_steps, d = data_loader(dataset)
    w = np.zeros([d]) #+ (1 if bias else 0)])
    b = 0.0
    dd = d+1 if bias else d
    input_normalizer = MDNEAN(theta_mu=theta_MDN, eta_v=eta_v, h_decay=h_decay)
    MSE_list = []
