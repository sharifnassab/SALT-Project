...
alg_name =  'NIDBD3_MetaPost_vs_Pre_K1' 
base_alg = 'LMS_MDN'
...
h_pre_norm = "steady" # "steady", "none", "alpha"
                            ...
                            for meta_post_norm in [
                                ...
                                for delta_norm in ["False"]: #"True", "False"
                                    for beta_pre_norm in ["none"]: #"steady", "none"
                                        for h_pre_norm in [ "alpha"]: #"steady", "none", "alpha"
                                            if beta_pre_norm=="steady" and delta_norm=="True" and h_pre_norm=="none": # already tested in NIDBD3_withMetaPostNorm
                                                continue
                                            if beta_pre_norm=="steady" and h_pre_norm=="steady": 
                                                continue
    ...
    for iteration in range(num_steps):
        ...
        if 1:
            ...
            if h_pre_norm == "steady":
                h_prenorm_coeff = np.sqrt(2*alpha[:-1]-alpha[:-1]**2)
            elif h_pre_norm == "none":
                h_prenorm_coeff = 1.0
            elif h_pre_norm == "alpha":
                h_prenorm_coeff = alpha[:-1]
            
            else:
                raise(ValueError)
            
            h = h * clip0(1-alpha[:-1]* xb**2) + h_prenorm_coeff * delta_tilde*xb
